...
var toggleStuff = function toggleStuff($tr) {
  //Toggling inputs
  $('input.mappingRolesEdit', $tr).toggleClass('hidden');
  $('.mappingRolesView', $tr).toggleClass('hidden'); //Toggling buttons
...
var renderRoleMappings = function renderRoleMappings() {
...
$(document).ready(function () {
  ...
  $('.mappingSaveBtn').click(function () {
    ...
    jagg.post("/site/blocks/settings/scope-mapping/ajax/scope-mapping.jag", {
      ...
      if (!json.error) {
        ...
        jagg.message({
          content: 'Successfully updated ' + scopeName,
          type: "info"
        });
      } else {
        jagg.message({
          content: 'Error updating ' + scopeName,
          type: "error"
        });
      }
    }, "json");
  ...
  $(document).on("click", '#role_mapping_add', function () {
    ...
    mappedRoleVal = mappedRoleVal.trim(); //update tenant-conf.json
    ...
    jagg.post("/site/blocks/settings/scope-mapping/ajax/scope-mapping.jag", {
      ...
      if (!json.error) {
        jagg.message({
          content: 'Successfully updated role mapping for ' + originalRoleVal,
          type: "info"
        });
      } else {
        jagg.message({
          content: 'Error while updating role mapping for ' + originalRoleVal,
          type: "error"
        });
      }
    }, "json");
    $("#original_role").val("");
    $("#mapped_role").val(""); //render role mappings table
    ...
    jagg.message({
      ...
      okCallback: function okCallback() {
        ...
        jagg.post("/site/blocks/settings/scope-mapping/ajax/scope-mapping.jag", {
          ...
          if (!json.error) {
            jagg.message({
              content: 'Successfully deleted role mapping for ' + originalRoleVal,
              type: "info"
            });
          } else {
            jagg.message({
              content: 'Error while deleting role mapping for ' + originalRoleVal,
              type: "error"
            });
          }
        }, "json");
