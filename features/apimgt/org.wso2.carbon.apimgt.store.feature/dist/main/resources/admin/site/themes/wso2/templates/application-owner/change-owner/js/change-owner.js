...
var changeOwner = function changeOwner() {
  if (!validateInputs()) {
    return;
  }
  ...
  jagg.post("/site/blocks/application-owner/change-owner/ajax/change-owner.jag", {
    ...
    if (result.error == false) {
      jagg.message({
        content: i18n.t("Successfully updated owner of the application " + document.getElementById('applicationName').value + " to " + document.getElementById('applicationOwner').value),
        type: "info",
        cbk: function cbk() {
