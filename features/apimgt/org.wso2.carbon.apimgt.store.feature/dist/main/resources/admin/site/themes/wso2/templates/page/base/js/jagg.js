...
(function () {
  ...
  jagg.message = function (params) {
    ...
    jagg.messageDisplay({
      content: params.content,
      title: i18n.t("Admin Portal - ") + type,
      buttons: [{
        name: "OK",
        cssClass: "btn btn-primary",
        cbk: function cbk() {
          $('#messageModal').modal('hide');
          if (params.cbk && typeof params.cbk == "function") params.cbk();
        }
      }]
    });
  };
  ...
  jagg.sessionExpired = function () {
    ...
    jagg.syncPost("/site/blocks/user/login/ajax/sessionCheck.jag", {
      action: "sessionCheck"
    }, function (result) {
      if (result != null) {
    ...
    if (jagg.sessionExpired()) {
      if (ssoEnabled) {
        ...
        if (params.e != undefined) {
          //Canceling the href call
          if (params.e.preventDefault) {
            params.e.preventDefault(); // otherwise set the returnValue property of the original event to false (IE)
          } else {
