...
(function (window, document, undefined) {
  var factory = function factory($, DataTable) {
    ...
    var Scroller = function Scroller(oDTSettings, oOpts) {
      ...
      this.s = $.extend(this.s, Scroller.oDefaults, oOpts); // Workaround for row height being read from height object (see above comment)
    ...
    /** @lends Scroller.prototype */
    {
       ...
       *    } );
       */
      "fnRowToPixels": function fnRowToPixels(rowIdx, intParse, virtual) {
        var pixels;
       ...
       *    } );
       */
      "fnPixelsToRow": function fnPixelsToRow(pixels, intParse, virtual) {
       ...
       *    } );
       */
      "fnScrollToRow": function fnScrollToRow(iRow, bAnimate) {
        var that = this;
        var ani = false;
        var px = this.fnRowToPixels(iRow); // We need to know if the table will redraw or not before doing the
       ...
       *      }, 3000 );
       */
      "fnMeasure": function fnMeasure(bRedraw) {
      ...
      "_fnConstruct": function _fnConstruct() {
        ...
        $(this.s.dt.nTableWrapper).addClass('DTS'); // Add a 'loading' indicator

        if (this.s.loadingIndicator) {
          this.dom.loader = $('<div class="DTS_Loading">' + this.s.dt.oLanguage.sLoadingRecords + '</div>').css('display', 'none');
       ...
       *  @private
       */
      _domain: function _domain(dir, val) {
        var heights = this.s.heights;
        var coeff; // If the virtual and physical height match, then we use a linear
      ...
      "_fnDrawCallback": function _fnDrawCallback() {
        ...
        this.s.skip = true; // Resize the scroll forcing element

        this._fnScrollForce(); // Reposition the scrolling for the updated virtual position if needed
        ...
        this.dom.scroller.scrollTop = iScrollTop; // Store positional information so positional calculations can be based
        ...
        if (this.s.dt.oFeatures.bStateSave && this.s.dt.oLoadedState !== null && typeof this.s.dt.oLoadedState.iScroller != 'undefined') {
          ...
          if (ajaxSourced && this.s.dt.iDraw == 2 || !ajaxSourced && this.s.dt.iDraw == 1) {
            setTimeout(function () {
              ...
              that.s.redrawTop = that.s.dt.oLoadedState.iScroller - heights.viewport / 2; // In order to prevent layout thrashing we need another
              // small delay
