...
var saveAlertTypes = function saveAlertTypes(alertTypesIDs, emailList, checkedValues) {
  jagg.post("/site/blocks/manage-alerts/ajax/manage-alerts.jag", {
    ...
    if (!result.error) {
      jagg.message({
        content: i18n.t("Successfully saved"),
        type: "info"
      });
      $("#unsubscribeBtn").show();
    } else {
...
var unSubscribeAlerts = function unSubscribeAlerts() {
  jagg.post("/site/blocks/manage-alerts/ajax/manage-alerts.jag", {
    action: "unSubscribe"
  }, function (result) {
    //console.log(result);
...
$(document).ready(function () {
  ...
  $("#saveBtn").click(function () {
    if ($(".token").hasClass("invalid")) {
      jagg.message({
        content: i18n.t("Could not save. You have entered an invalid email address."),
        type: "error"
      });
    } else {
      var notChecked = [],
      ...
      if (checkedValues.length > 0) {
        ...
        if (emailList) {
          ...
          jagg.message({
            content: i18n.t("Please enter at least one email address"),
            type: "error"
          });
        }
      } else {
        jagg.message({
          content: i18n.t("Please select at least one alert type"),
          type: "error"
        });
      }
    }
