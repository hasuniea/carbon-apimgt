...
var index = 0;
$.fn.editableform.buttons = '<button type="submit" class="btn btn-primary btn-sm editable-submit">' + '<i class="fw fw-check"></i>' + '</button>' + '<button type="button" class="btn btn-secondary btn-sm editable-cancel">' + '<i class="fw fw-cancel"></i>' + '</button>';
...
var changeStatusIcon = function changeStatusIcon(flowId, type, checkBoxOb) {
  var spanId;
  var iconId;
...
$(document).ready(function () {
  ...
  $.validator.addMethod('requestCountValidator', function (value, element) {
    return !isNaN(value);
  }, "Error: Value Entered for Request Count is not valid!!"); //validate time is numeric

  $.validator.addMethod('timeValidator', function (value, element) {
    return !isNaN(value);
  }, "Error: Value entered for unit time is not valid"); //validate bandwidth is numberic
...
var showAdvanceOperation = function showAdvanceOperation(operation, button, id) {
...
var onIpConditionChange = function onIpConditionChange(id, optionTextOb) {
...
var onExecutionFlowTypeChange = function onExecutionFlowTypeChange(id, optionTextOb) {
  var selectedText = $(optionTextOb).val();
...
var onIPChange = function onIPChange(IPtextEle) {
  //validate the IP
...
var onEndIPChange = function onEndIPChange(IPtextEle) {
...
var valid_block = function valid_block(block) {
...
var validIPRange = function validIPRange(startIP, endIP) {
  ...
  if (startIP.includes(".") && endIP.includes(".")) {
    ...
    var startIPBlocks = startIP.split(":");
    var endIPBlocks = endIP.split(":"); //Check from the begining which is greater. Go through 8 blocks
...
var countSemicolon = function countSemicolon(ip) {
  var count = 0;
...
var getFullAddress = function getFullAddress(ip) {
  var temp = ip;
  var colonCount = countSemicolon(temp); //full representation has 7 colons (:)
...
var onDateConditionChange = function onDateConditionChange(id, optionTextOb) {
  var selectedText = $(optionTextOb).val();
...
var removeQueryParamRow = function removeQueryParamRow(rowObj, flowId) {
  $(rowObj).closest('tr').remove();
  var rowCount = $('#query-param-value-table-content-' + flowId + ' tr').length; //Using 1 here as even row is removed above the row count will not set to 0 during this flow
...
var addQueryParam = function addQueryParam(id) {
...
var removeHeaderRow = function removeHeaderRow(rowObj, flowId) {
  $(rowObj).closest('tr').remove();
  var rowCount = $('#header-value-table-content-' + flowId + ' tr').length; //Using 1 here as even row is removed above the row count will not set to 0 during this flow
...
var removeJwtClaimRow = function removeJwtClaimRow(rowObj, flowId) {
  $(rowObj).closest('tr').remove();
  var rowCount = $('#jwt-claim-value-table-content-' + flowId + ' tr').length; //Using 1 here as even row is removed above the row count will not set to 0 during this flow
...
var addJwtClaim = function addJwtClaim(id) {
...
var loadPolicy = function loadPolicy(policyName) {
...
var addPolicyToBackend = function addPolicyToBackend() {
  ...
  var policyLevel = $("#policy-level option:selected").val(); //var defaultPolicyType = $("#default-policy-level option:selected").val();
  ...
  for (var i = 0; i < apiPolicyNew.executionFlows.length; i++) {
    ...
    for (var j = 0; j < apiPolicyNew.executionFlows[i].conditions.length; j++) {
      if (apiPolicyNew.executionFlows[i].conditions[j].type == "IP") {
        checked = $('#ip-condition-checkbox-' + executionFlowId).is(':checked'); //Ip condition related properties
