...
$(document).ready(function () {
  ...
  $('.js_completeBtn').click(function () {
    ...
    btn.attr("disabled", "disabled");
    jagg.post("/site/blocks/task-manager/ajax/task.jag", {
      action: "bpmnCompleteTask",
      status: status,
      taskId: taskId,
      taskName: "APIStateApprovalTask",
  ...
  $('.js_assignBtn').click(function () {
    ...
    jagg.post("/site/blocks/task-manager/ajax/task.jag", {
      ...
      if (!json.error) {
        ...
        $('#js_completeBtn' + iteration).show();
        btn.remove();
        window.location.reload(); // $('#status'+iteration).text("RESERVED");
      } else {
        jagg.showLogin();
      }
    }, "json");
  ...
  $('#appreg-tasks').datatables_extended({
    "fnDrawCallback": function fnDrawCallback() {
      if (this.fnSettings().fnRecordsDisplay() <= $("#appreg-tasks_length option:selected").val() || $("#appreg-tasks option:selected").val() == -1) $('#appreg-tasks_paginate').hide();else $('#appreg-tasks_paginate').show();
    },
