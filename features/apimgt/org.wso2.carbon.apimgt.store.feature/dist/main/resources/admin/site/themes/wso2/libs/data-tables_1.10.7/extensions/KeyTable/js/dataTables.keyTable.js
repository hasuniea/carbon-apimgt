...
(function (window, document, undefined) {
  var factory = function factory($, DataTable) {
    ...
    KeyTable = function KeyTable(oInit) {
      /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
       * API parameters
       * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
      ...
      this.fnSetPosition = function (x, y) {
        if (_typeof(x) == 'object' && x.nodeName) {
          _fnSetFocus(x);
        } else {
      ...
      function _fnEventAddTemplate(sKey) {
        ...
        return function (x, y, z) {
          if ((x === null || typeof x == "number") && (y === null || typeof y == "number") && typeof z == "function") {
            _fnEventAdd(sKey, x, y, z);
          } else if (_typeof(x) == "object" && typeof y == "function") {
      ...
      function _fnEventRemoveTemplate(sKey) {
         ...
         */
        return function (x, y, z) {
          if ((x === null || typeof arguments[0] == "number") && (y === null || typeof arguments[1] == "number")) {
      ...
      function _fnEventRemove(sType, x, y, fn) {
        ...
        for (var i = 0, iLen = _oaoEvents[sType].length; i < iLen - iCorrector; i++) {
          if (typeof fn != 'undefined') {
            if (_oaoEvents[sType][i - iCorrector].x == x && _oaoEvents[sType][i - iCorrector].y == y && _oaoEvents[sType][i - iCorrector].fn == fn) {
              _oaoEvents[sType].splice(i - iCorrector, 1);

              iCorrector++;
            }
          } else {
            if (_oaoEvents[sType][i - iCorrector].x == x && _oaoEvents[sType][i - iCorrector].y == y) {
      ...
      function _fnEventFire(sType, x, y) {
        ...
        for (var i = 0; i < aEvents.length; i++) {
          if (aEvents[i].x == x && aEvents[i].y == y || aEvents[i].x === null && aEvents[i].y == y || aEvents[i].x == x && aEvents[i].y === null || aEvents[i].x === null && aEvents[i].y === null) {
      ...
      function _fnSetFocus(nTarget, bAutoScroll) {
        ...
        if (bAutoScroll) {
          ...
          if (_oDatatable && typeof oSettings.oScroll != 'undefined' && (oSettings.oScroll.sX !== "" || oSettings.oScroll.sY !== "")) {
      ...
      function _fnCoordsFromCell(n) {
        if (_oDatatable) {
          return [$('td', n.parentNode).index(n), $('tr', n.parentNode.parentNode).index(n.parentNode) + _oDatatable._iDisplayStart];
        } else {
          return [$('td', n.parentNode).index(n), $('tr', n.parentNode.parentNode).index(n.parentNode)];
        }
      }
      /*
