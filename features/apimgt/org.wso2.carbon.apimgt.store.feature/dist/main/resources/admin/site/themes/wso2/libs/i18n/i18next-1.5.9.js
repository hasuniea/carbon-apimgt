...
(function () {
  ...
  var root = this,
      ...
      languages = []; // Export the i18next object for **CommonJS**. 
  ...
  function _ajax(options) {
    ...
    var ajax = function ajax(method, url, options, callback) {
      ...
      if (method === 'GET') {
        ...
        url += queryString !== '?' ? queryString : ''; // Make a JSONP call if neccessary.
  ...
  var _cookie = {
    create: function create(name, value, minutes) {
      var expires;
  ...
  function init(options, cb) {
    ...
    f.extend(o, options); // create namespace object if namespace is passed in as string
      ...
      if (value !== undefined) {
        if (typeof value === 'string') {
          ...
          if (!o.returnObjectTrees && !options.returnObjectTrees) {
            value = 'key \'' + ns + ':' + key + ' (' + l + ')\' ' + 'returned a object instead of string.';
            f.log(value);
          } else {
            for (var m in value) {
      ...
      if (!options.dynamicLoad) {
        ...
        f.ajax({
          ...
          error: function error(xhr, status, _error) {
            ...
            cb('failed loading resource.json error: ' + _error);
          },
          dataType: "json",
