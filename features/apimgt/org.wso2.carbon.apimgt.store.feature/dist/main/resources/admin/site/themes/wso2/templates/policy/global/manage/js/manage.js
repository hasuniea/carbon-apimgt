...
var deleteGlobalPolicy = function deleteGlobalPolicy(policyObject) {
  jagg.message({
...
$(function () {
  ...
  $('#global-policy').datatables_extended({
    "fnDrawCallback": function fnDrawCallback() {
      if (this.fnSettings().fnRecordsDisplay() <= $("#global-policy_length option:selected").val() || $("#global-policy_length option:selected").val() == -1) $('#global-policy_paginate').hide();else $('#app-policy_paginate').show();
    },
    "aoColumns": [null, null, {
      "bSortable": false
    }, {
