...
var addAppPolicy = function addAppPolicy() {
  if (!validateInputs()) {
    return;
  }
...
function addCustomAttribute(element, count) {
  var elementId = element.attr('id');
  element.parent().append('<tr id="attribute' + count + '">' + '<td><div class="clear"></div></td>' + '<td><input type="text" id="attributeName' + count + '" name="attributeName' + count + '" placeholder="Attribute Name"/></td>' + '<td><input type="text" id="attributeValue' + count + '" name="attributeValue' + count + '" placeholder="Value"/></td>' + '<td class="delete_resource_td"><a  id="attributeDelete' + count + '"  href="javascript:removeCustomAttribute(' + count + ')"><i class="icon-trash"></i></a></td>' + '</tr>');
}
